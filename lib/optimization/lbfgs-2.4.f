...
c
c       Linpack Library ... lbfg_dcopy, lbfg_ddot.
c
c
c     References:
c
...
c     ************
     ...
     +                 iword,nfree,nact,ileave,nenter
      double precision theta,fold,lbfg_ddot,dr,rr,tol,dpmeps,
...
c                                          restore the previous iterate.
               call lbfg_dcopy(n,t,1,x,1)
               call lbfg_dcopy(n,r,1,g,1)
               f = fold
            endif
            goto 999
         endif
...
c     Compute d=newx-oldx, r=newg-oldg, rr=y'y and dr=y's.
      ...
      do 42 i = 1, n
         r(i) = g(i) - r(i)
  42  continue
      rr = lbfg_ddot(n,r,1,r,1)
      if (stp .eq. one) then
         dr = gd - gdold
         ddum = -gdold
      else
         dr = (gd - gdold)*stp
         call lbfg_dscal(n,stp,d,1)
         ddum = -gdold*stp
      endif
...
c
c       Linpack ... lbfg_dscal lbfg_dcopy, lbfg_daxpy.
c
c
c     References:
c
...
c     ************
     ...
     +                 tu,tl,wmc,wmp,wmw,lbfg_ddot,tj,tj0,neggi,sbgnrm,
     +                 f2_org
      double precision one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c                   complete the initialization of p for theta not= one.
         call lbfg_dscal(col,theta,p(col+1),1)
      endif
...
c        temporarily set f1 and f2 for col=0.
      ...
      if (col .gt. 0) then
c                          update c = c + dt*p.
         call lbfg_daxpy(col2,dt,p,1,c,1)
...
c           compute (wbp)Mc, (wbp)Mp, and (wbp)M(wbp)'.
         ...
         if (info .ne. 0) return
         wmc = lbfg_ddot(col2,c,1,v,1)
         wmp = lbfg_ddot(col2,p,1,v,1)
         wmw = lbfg_ddot(col2,wbp,1,v,1)

c           update p = p - dibp*wbp.
         call lbfg_daxpy(col2,-dibp,wbp,1,p,1)
...
c       the variables whose breakpoints haven't been reached.
      ...
      call lbfg_daxpy(n,tsum,d,1,xcp,1)
...
c       which will be used in computing r = Z'(B(x^c - x) + g).
      ...
      if (col .gt. 0) call lbfg_daxpy(col2,dtm,p,1,c,1)
      if (iprint .gt. 100) write (6,1010) (xcp(i),i = 1,n)
...
c     Subprograms called:
c
c       Linpack ... lbfg_dcopy, lbfg_dpofa, dtrsl.
c
c
c     References:
c       [1] R. H. Byrd, P. Lu, J. Nocedal and C. Zhu, ``A limited
...
c     ************
     ...
     +                 col2,pbegin,pend,dbegin,dend,upcl
      double precision lbfg_ddot,temp1,temp2,temp3,temp4
      double precision zero
      parameter        (zero=0.0d0)
...
c                                 shift old part of WN1.
            do 10 jy = 1, m - 1
               js = m + jy
               call lbfg_dcopy(m-jy,wn1(jy+1,jy+1),1,wn1(jy,jy),1)
               call lbfg_dcopy(m-jy,wn1(js+1,js+1),1,wn1(js,js),1)
               call lbfg_dcopy(m-1,wn1(m+2,jy+1),1,wn1(m+1,jy),1)
  10        continue
         endif
...
c                          with L' stored in the upper triangle of wn.
      call lbfg_dpofa(wn,m2,col,info)
      if (info .ne. 0) then
         info = -1
         return
      endif
...
c        upper triangle of (2,2) block of wn.
      ...
      do 72 is = col+1, col2
         do 74 js = is, col2
         wn(is,js) = wn(is,js) + lbfg_ddot(col,wn(1,is),1,wn(1,js),1)
  74        continue
  72     continue

c     Cholesky factorization of (2,2) block of wn.

      call lbfg_dpofa(wn(col+1,col+1),m2,col,info)
      if (info .ne. 0) then
         info = -2
         return
      endif
...
c     **********
      ...
      integer          i
      double precision lbfg_ddot,a1,a2
...
c     Determine the maximum step length.
  ...
  43        continue
      ...
      call lbfg_dcopy(n,x,1,t,1)
      call lbfg_dcopy(n,g,1,r,1)
      fold = f
      ifun = 0
      iback = 0
      csave = 'START'
 556  continue
      gd = lbfg_ddot(n,g,1,d,1)
      if (ifun .eq. 0) then
         gdold=gd
         if (gd .ge. zero) then
...
c     Subprograms called:
c
c       Linpack ... lbfg_dcopy, lbfg_ddot.
c
c
c                           *  *  *
c
...
c     Update matrices WS and WY.
      ...
      call lbfg_dcopy(n,d,1,ws(1,itail),1)
      call lbfg_dcopy(n,r,1,wy(1,itail),1)
...
c                              move old information
         do 50 j = 1, col - 1
            call lbfg_dcopy(j,ss(2,j+1),1,ss(1,j),1)
            call lbfg_dcopy(col-j,sy(j+1,j+1),1,sy(j,j),1)
  50     continue
      endif
...
c                                             and the last column of SS:
      pointr = head
      do 51 j = 1, col - 1
         sy(col,j) = lbfg_ddot(n,d,1,wy(1,pointr),1)
         ss(j,col) = lbfg_ddot(n,ws(1,pointr),1,d,1)
         pointr = mod(pointr,m) + 1
  51  continue
      if (stp .eq. one) then
         ss(col,col) = dtd
...
c====================== The end of timer ===============================
      ...
      double precision function lbfg_dnrm2(n,x,incx)
      integer n,incx
      double precision x(n)
c     **********
c
c     Function lbfg_dnrm2
c
...
c
c       double precision function lbfg_dnrm2(n,x,incx)
c
c     where
c
...
c     **********
   ...
   10 continue
      ...
      do 20 i = 1, n, incx
         lbfg_dnrm2 = lbfg_dnrm2 + (x(i)/scale)**2
   20 continue

      lbfg_dnrm2 = scale*sqrt(lbfg_dnrm2)
...
c====================== The end of dpmeps ==============================
      ...
      subroutine lbfg_daxpy(n,da,dx,incx,dy,incy)
c
...
c====================== The end of lbfg_daxpy ===============================

      subroutine lbfg_dcopy(n,dx,incx,dy,incy)
c
...
c====================== The end of lbfg_dcopy ===============================

      double precision function lbfg_ddot(n,dx,incx,dy,incy)
c
...
c====================== The end of lbfg_ddot ================================

      subroutine lbfg_dpofa(a,lda,n,info)
      integer lda,n,info
      double precision a(lda,*)
c
c     lbfg_dpofa factors a double precision symmetric positive definite
c     matrix.
c
c     lbfg_dpofa is usually called by dpoco, but it can be called
c     directly with a saving in time if  rcond  is not needed.
c     (time for dpoco) = (1 + 18/n)*(time for lbfg_dpofa) .
c
c     on entry
c
...
c
         do 30 j = 1, n
            ...
            do 10 k = 1, jm1
               t = a(k,j) - lbfg_ddot(k-1,a(1,k),1,a(1,j),1)
               t = t/a(k,k)
               a(k,j) = t
               s = s + t*t
   10       continue
...
c====================== The end of lbfg_dpofa ===============================

      subroutine  lbfg_dscal(n,da,dx,incx)
c
...
c     subroutines and functions
c
c     blas lbfg_daxpy,lbfg_ddot
c     fortran mod
c
c     internal variables
c
      double precision lbfg_ddot,temp
      integer case,j,jj
c
...
c
   20    continue
            ...
            do 30 j = 2, n
               temp = -b(j-1)
               call lbfg_daxpy(n-j+1,temp,t(j,j-1),1,b(j),1)
               b(j) = b(j)/t(j,j)
   30       continue
   40       continue
         go to 140
...
c
   50    continue
            ...
            do 60 jj = 2, n
               j = n - jj + 1
               temp = -b(j+1)
               call lbfg_daxpy(j,temp,t(1,j+1),1,b(1),1)
               b(j) = b(j)/t(j,j)
   60       continue
   70       continue
         go to 140
...
c
   80    continue
            ...
            do 90 jj = 2, n
               j = n - jj + 1
               b(j) = b(j) - lbfg_ddot(jj-1,t(j+1,j),1,b(j+1),1)
               b(j) = b(j)/t(j,j)
   90       continue
  100       continue
         go to 140
...
c
  110    continue
            ...
            if (n .lt. 2) go to 130
            do 120 j = 2, n
               b(j) = b(j) - lbfg_ddot(j-1,t(1,j),1,b(1),1)
               b(j) = b(j)/t(j,j)
  120       continue
  130       continue
  140    continue
